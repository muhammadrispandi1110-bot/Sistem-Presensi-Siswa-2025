             ...
             <div className="space-y-6">
                ...
                {supabase && classes.length === 0 && !isLoading && (
                    ...
                    <div className="p-4 rounded-lg border bg-slate-800 border-slate-700"><p className="text-slate-300 mb-4">Database Anda kosong. Klik untuk mengisi data awal (4 kelas & siswanya).</p>
                        ...
                        {isSyncing ? (<svg className="animate-spin h-5 w-5 text-white" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A8 8 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>) : (<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M3 17a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zm6-10a1 1 0 011-1h2a1 1 0 110 2H10a1 1 0 01-1-1z" clipRule="evenodd" /><path d="M4 5a2 2 0 012-2h8a2 2 0 012 2v8a2 2 0 01-2 2H6a2 2 0 01-2-2V5z" /></svg>)}
        ...
        <div className="flex-1 p-4 sm:p-6 overflow-y-auto view-transition">
            ...
            {activeClass.students.map((student, idx) => {
                ...
                <div key={student.id} className="dark-card p-4 rounded-xl flex flex-col justify-between">
                    <div><p className="text-slate-400 text-sm">{idx + 1}.</p><p className="font-semibold text-slate-200 mt-1 truncate">{student.name}</p></div>
                    <div className="grid grid-cols-4 gap-2 mt-4">
                        {(['H', 'S', 'I', 'A'] as AttendanceStatus[]).map(s => (<button key={s} onClick={() => !isFutureDate(currentDate) && handleAttendanceChange(student.id, dateStr, s)} disabled={isFutureDate(currentDate)} className={`h-10 text-sm font-bold rounded-md transition-all ${status === s ? DARK_STATUS_COLORS[s] : 'bg-slate-800 hover:bg-slate-700'} ${isFutureDate(currentDate) ? 'opacity-50 cursor-not-allowed' : ''}`}>{s}</button>))}
                    </div>
                </div>)
            })}
            </div>
